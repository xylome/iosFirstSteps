...
extension PlaySoundViewController: AVAudioPlayerDelegate {
    ...
    func playSound(rate rate: Float? = nil, pitch: Float? = nil, echo: Bool = false, reverb: Bool = false) {
        if chipmunkButton.enabled == false && chipmunkButton2.enabled == false {
            print("exiting : apparently already playing a sound")
            return
        }
    ...
    func setPlayButtonEnabled(state :Bool) {
        ...
        snailButton2.enabled = state
        rabbitButton2.enabled = state
        chipmunkButton2.enabled = state
        vadorButton2.enabled = state
        echoButton2.enabled = state
        reverbButton2.enabled = state
